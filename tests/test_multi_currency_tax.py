"""
Test suite for tenant currency and tax/VAT configuration functionality.

Tests that invoices use tenant's default currency and tax calculation.
"""
from decimal import Decimal


def test_create_invoice_uses_tenant_default_currency(client, auth_headers):
    ...
    data = response.json()
    # Invoice uses tenant's default currency (NGN)
    assert data["currency"] == "NGN"


def test_invoice_tax_calculation_with_tenant_tax_rate(
...
def test_invoice_no_tax_when_tenant_has_no_tax_rate(
    client, auth_headers, db_session, test_tenant
):
...
def test_invoice_with_multiple_items_tax_calculation(
    client, auth_headers, db_session, test_tenant
):
...
def test_list_invoices_shows_currency(client, auth_headers):
    ...
    # Create an invoice (uses tenant's default currency)
    create_response = client.post(
        "/api/v1/invoices/",
        json={
            "customer_name": "Currency List Customer",
            "issue_date": "2024-01-15",
            "items": [
                {
                    "description": "Product",
    ...
    invoice = next(
        (inv for inv in data["items"] if inv["id"] == invoice_id),
        None
    )
...
def test_zero_tax_rate_means_no_tax(
    client, auth_headers, db_session, test_tenant
):
