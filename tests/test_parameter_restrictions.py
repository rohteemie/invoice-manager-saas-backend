...
def test_regular_user_cannot_update_plan_type(
    ...
    # because regular users (owners) are not allowed to update plan_type
    assert response.status_code == 403
    assert "only super admins can change the plan type" in response.text.lower()
